...
def test_load_dataset(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(module.pd, "read_csv", lambda _path: pd.read_csv(dataset_path))
...
def test_train_xgb_model(tmp_path, monkeypatch):
    module = import_project_v7(tmp_path)
    df = pd.DataFrame({"Resume": ["a", "b", "c", "d"], "Category": ["X", "Y", "X", "Y"]})
    monkeypatch.setattr(module, "load_dataset", lambda: (df, df["Resume"], np.array([0, 1, 0, 1]), df["Category"].values, module.LabelEncoder().fit(df["Category"])))
